...
select value from (select value, IF(value=0, ~0, value) as zero  from storehouses_products ORDER BY zero) as goods;
...
SELECT AVG(age) FROM (SELECT YEAR(CURRENT_TIMESTAMP) - YEAR(birthday) as age FROM profiles) AS average_age;
...
-- тут date=1 это понедельник, date=2 это вторник и т.д
